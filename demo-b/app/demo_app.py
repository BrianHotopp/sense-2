...
def generate_embedding():
    ...
    for word in wv.get_words():
        ...
        write_db("INSERT INTO e_vectors (e_id, word, vector) VALUES (?, ?, ?)", (e_id, word, v))
...
def generate_alignment():
    ...
    a_id = write_db_ret_last("INSERT INTO alignments (name, description, e1_id, e2_id) VALUES (?, ?, ?, ?)", (name, description, e1_id, e2_id))
    ...
    # get datbase cursor
    c = get_db().cursor()
    ps_args = [(a_id, True, a.common[i], pickle.dumps(a.v1[i])) for i in range(len(a.common))]
    c.executemany(ps, ps_args)
    ps_args = [(a_id, False, a.common[i], pickle.dumps(a.v2[i])) for i in range(len(a.common))]
    c.executemany(ps, ps_args)
    # insert into shifts
    ...
    ps_args = [(a_id, a.common[i], a.common[j], a.dists[i,j]) for i in range(len(a.common)) for j in range(len(a.common))]
    c.executemany(ps, ps_args)
    # commit changes
    get_db().commit()
    end = time.time()
...
def get_alignments():
    ...
    if query_db("SELECT * FROM embeddings WHERE id = ?", (e2_id,), one=True) is None:
        return jsonify({'message': f'Invalid embedding id: {e2_id}'}), 400
    # get the alignments
    r = query_db("SELECT * FROM alignments WHERE e1_id = ? AND e2_id = ?", (e1_id, e2_id))
...
def get_top_shifted_words():
    ...
    r = query_db("SELECT word, shift FROM shifts WHERE a_id = ? ORDER BY shift DESC LIMIT ?", (a_id, num_words))
...
def get_context():
    ...
    if 'first' not in d:
        return jsonify({'message': 'No first flag supplied, cannot know if target word is in first or second context of alignment'}), 400
    first = d['first']
    ...
    if first:
        ...
        r = query_db("SELECT word2 as neighbor, dist FROM dists WHERE a_id = ? AND word1 = ? ORDER BY dist ASC LIMIT ?", (a_id, word, n_neighbors))
    else:
        # our target word is in the second context
        # get the closest n neighbors in the first context
        r = query_db("SELECT word1 as neighbor, dist FROM dists WHERE a_id = ? AND word2 = ? ORDER BY dist ASC LIMIT ?", (a_id, word, n_neighbors))
    # get the vectors for the neighbors
    v = []
    for i in range(len(r)):
        if first:
            vecb = query_db("SELECT vector FROM a_vectors WHERE a_id = ? AND first = ? AND word = ?", (a_id, False, r[i]['neighbor']), one=True)
            # unpack the vector
            v.append(pickle.loads(vecb['vector'])) 
        else:
            vecb = query_db("SELECT vector FROM a_vectors WHERE a_id = ? AND first = ? AND word = ?", (a_id, True, r[i]['neighbor']), one=True)
            # unpack the vector
            v.append(pickle.loads(vecb['vector']))
    # return the neighbors and their vectors
    v = np.array(v)
    v_2d = PCA(n_components=2).fit_transform(v)
    return jsonify({'message': 'Context retrieved', 'neighbors': r, 'vectors': v_2d.tolist()}), 200
