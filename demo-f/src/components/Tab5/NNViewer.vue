...
onMounted(() => {
  ...
  if (store.selectedWord != null) {
    ...
    getSingleMostShiftedWord(props.al)
      .then((word) => {
...
function swapContext() {
  ...
  getContext(props.al, target_word.value, nfirst, num_neighbors.value).then(
    (data) => {
      // error handling
      if (data.error) {
        console.log("Failed to get the context from the api");
...
function updateSettings(new_settings) {
  ...
  getContext(
    props.al,
    new_settings.target_word,
    first.value,
    new_settings.num_neighbors
  ).then((data) => {
    // error handling
    if (data.error) {
      console.log("Failed to get the context from the api");
...
<template>
  <div class="card p-3 mb-3">
    ...
    <div class="row">
      <div class="col">
        <p>
          Viewing words from {{ secondctx }} that are most similar in meaning to
          <b>{{ target_word }}</b> from {{ firstctx }}.
        </p>

        <button class="btn btn-primary mb-3" @click="swapContext">
          Swap Context
        </button>
        <!--form  -->
        <div class="card mb-3">
          <div class="form-group row p-3">
            <label for="num_neighbors" class="col-sm-2 col-form-label"
              >Number of neighbors</label
            >
            <div class="col-sm-2">
              <input
                type="number"
                class="form-control"
                id="num_neighbors"
                v-model="formdata.num_neighbors"
                min="1"
                max="100"
              />
            </div>
            ...
            <div class="col-sm-2">
              <button class="btn btn-primary" @click="updateSettings(formdata)">
                Update Settings
              </button>
            </div>
          </div>
